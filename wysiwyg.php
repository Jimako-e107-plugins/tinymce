...
tinymce.init({
	...
	"external_plugins": {"e107":"/e107_plugins/tinymce/plugins/e107/plugin.js","example":"/e107_plugins/tinymce/plugins/example/plugin.js"},
	"image_advtab": true,
...
tinymce.init({
    ...
    external_plugins: {
        "example": "{e_PLUGIN_ABS}tinymce/plugins/example/plugin.min.js",
        "e107" : "{e_PLUGIN_ABS}tinymce/plugins/e107/plugin.js"
    },
    menubar: "edit view format insert table tools",
...
*/
	...
	function tinymce_lang()
	{
		...
		$jsFile = e_PLUGIN . 'tinymce/langs/' . $tinylang[$lang] . '.js';
	...
	function getExternalPlugins($data)
	{
		...
		foreach($tmp as $val)
		{
			$ext[$val] = e_PLUGIN_ABS."tinymce/plugins/".$val."/plugin.js";
		}
	...
	//	$fl = e107::getFile();
		...
		//	'editor_selector'   => 'advancedEditor',
			...
			'language_url'      => SITEURLBASE.e_PLUGIN_ABS."tinymce/langs/" . $tinyMceLanguage . ".js"
		);

	//	if(e_ADMIN_AREA)
		{
	//		$ret['skin_url']     = e_PLUGIN_ABS.'tinymce/skins/eskin';
		}
...
*/
	...
	//	$ret['skin']                    = 'e107admin';
	//	$ret['skin_url']                = SITEURLBASE.e_PLUGIN_ABS.'tinymce/skins/e107admin';
...
/*
		$editorCSS = array(
			...
			e_PLUGIN_ABS.'tinymce/editor.css',
		);
*/
        ...
        $sitetheme = e107::getPref('sitetheme');
         ...
         if(class_exists('theme')){
           $themeArray = new theme;
 
        }  
        $editorCSS[0] =  e_PLUGIN_ABS . 'tinymce/editor.css';
        foreach($themeArray->theme_css_array AS $key => $css) {
              $editorCSS[$key] = $css;
        }   
     ...
     /*
		...
		$editorCSS = array(
			...
			2 => e_PLUGIN_ABS . 'tinymce/editor.css',
...
/*
	...
	{
	//	$templatePath = (is_readable(THEME."templates/tinymce/".$template)) ? THEME."templates/tinymce/".$template : e_PLUGIN."tinymce/templates/".$template;
	...
	private function getSnippets()
	{
		...
		{
			$path = e_PLUGIN."tinymce/snippets";
			$base = e_PLUGIN_ABS."tinymce/snippets";
		}
		...
		foreach($files as $f)
		{
			...
			if(!empty($m[1]))
			{
			//	$url = e_PLUGIN_ABS."tinymce/snippets/".$f['fname'];
